...
export function PlayButtonSkip({
  ...
  const explicitEnd = typeof endTime === "number" && Number.isFinite(endTime) ? endTime : undefined;
  ...
  useEffect(() => {
    ...
    const onTimeUpdate = () => {
      ...
      const denom = Math.max(0.000001, (hasEnd ? effectiveRangeEnd : el.duration || 0) - rangeStart);
  ...
  const handleSeek = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    const rangeEnd: number = hasEnd ? effectiveRangeEnd : (duration || el.duration || rangeStart);
  ...
  const handleDragMove = (e: MouseEvent) => {
    ...
    if (!audio) return;
    const rangeEnd: number = hasEnd ? effectiveRangeEnd : (duration || audio.duration || rangeStart);
    const target = rangeStart + pct * Math.max(0, rangeEnd - rangeStart);
    try {
  ...
  const handleKeySeek = (e: React.KeyboardEvent<HTMLDivElement>) => {
    ...
    if (e.key === "ArrowRight") {
      ...
      setCurrentTime(next);
      setProgress(
        Math.max(0, Math.min(1, (next - rangeStart) / Math.max(0.000001, effectiveRangeEnd - rangeStart)))
      );
      e.preventDefault();
      ...
      setCurrentTime(prev);
      setProgress(
        Math.max(0, Math.min(1, (prev - rangeStart) / Math.max(0.000001, effectiveRangeEnd - rangeStart)))
      );
      e.preventDefault();
    }
  };
    ...
    <div className="flex w-full flex-col items-center gap-1 py-1">
      ...
      <div className="mt-2 w-full">
        <div className="mb-1 flex items-center justify-between text-[10px] text-muted-foreground">
          <span className="rounded border border-border bg-card/70 px-1.5 py-0.5" aria-label="Current time">
            {fmt(currentTime)}
          </span>
          <span className="rounded border border-border bg-card/70 px-1.5 py-0.5" aria-label="End time">
